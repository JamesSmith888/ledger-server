...
public class TransactionMCP {
    ...
    @Tool(description = """
            ...
            - transactionDateTime: Transaction date time (optional, format: yyyy-MM-dd HH:mm:ss, defaults to now)
            ...
            Workflow:
            1. Validate required parameters (name, amount, type, createdByUserId)
    ...
    public String createTransaction(String name, String description, BigDecimal amount,
        ...
        log.info("Creating transaction: name={}, amount={}, type={}, userId={}", name, amount, type, createdByUserId);

        LocalDateTime dateTime = transactionDateTime != null
                ? LocalDateTime.parse(transactionDateTime, FORMATTER)
                : null;

        var transaction = transactionService.create(name, description, amount, type, dateTime, ledgerId, createdByUserId);
    ...
    public String updateTransaction(Long id, String name, String description, BigDecimal amount,
                                    Integer type, String transactionDateTime) {
        log.info("Updating transaction: id={}, name={}, amount={}, type={}", id, name, amount, type);

        LocalDateTime dateTime = transactionDateTime != null
                ? LocalDateTime.parse(transactionDateTime, FORMATTER)
                : null;

        var transaction = transactionService.update(id, name, description, amount, type, dateTime);
    ...
    @Tool(description = """
            ...
            5. Return formatted statistics
            """)
    public String calculateUserStatistics(Long createdByUserId) {
        log.info("Calculating statistics for user: {}", createdByUserId);
